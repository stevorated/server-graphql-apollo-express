...
export default {
  Query: {
    getMonthsEvents: async (
      root,
      { month, limit, skip = 0, sort = 1 },
      { req },
      info
    ) => {
      ...
      return Event.find(
        ...
        null,
        { sort: { startDate: sort }, limit: controledLimit, skip }
      )
    },
    getMyMonthsEvents: async (
      root,
      { month, limit, skip = 0, sort = 1 },
      { req },
      info
    ...
    getMyEvents: async (
      root,
      { limit = 6, skip = 0, sort = 1, past, followed, suggested },
      { req },
      info
    ) => {
      const session = req.session.passport
    ...
    getMyEventsFeed: async (
      root,
      { limit = 6, skip = 0, sort = 1 },
      { req },
      info
    ) => {
      const session = req.session.passport
      ...
      if (sort !== 1 && sort !== -1)
        throw new UserInputError(`invalid sort must be 1 or -1`)
      if (!ObjectId.isValid(userId)) throw new UserInputError(`invalid ID`)
      return Event.find({ createdBy: ObjectId(userId) }, null, {
        sort: { createdAt: sort },
        limit: controledLimit,
        skip
      })
    ...
    getUsersEvents: async (
      ...
      return Event.find({ createdBy: ObjectId(id) }, null, {
        sort: { startDate: sort },
        limit: controledLimit,
        skip
      })
  ...
  Mutation: {
    ...
    followEvent: async (root, args, { req }, info) => {
      ...
      // console.log(myUser)
      if (
        myUser.followingEvents &&
        myUser.followingEvents.includes(eventToFollow.id)
      ) {
        // console.log('ya alla')
        await User.findByIdAndUpdate(
          myUser.id,
          { $pull: { followingEvents: eventToFollow.id } },
          { new: true }
        )
        ...
        await Notification.create({
          ...
          show: false,
          body: `unfollowed event, name: ${eventToFollow.name} id: ${
            args.event
          }`,
          type: 'EventFollowers',
      ...
      await User.findByIdAndUpdate(
        myUser.id,
        { $push: { followingEvents: eventToFollow.id } },
        { new: true }
      )
      const updatedEventToFollow = await Event.findByIdAndUpdate(
        args.event,
        {
  ...
  try {
    ...
    switch (type) {
      case 'eventImages':
        ...
        await Joi.validate({ size: sizeToNum, mimetype }, uploadImage(size), {
          abortEarly: false
        })
