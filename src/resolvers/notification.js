...
export default {
  Mutation: {
    createNotification: async (root, args, { req }, info) => {
      ...
      let to = null
      // if (!args.post && !args.event) {
      //   return new UserInputError(`Whachu talking 'bout?! `)
      // }
      if (args.post) {
  ...
  Query: {
    getMyNotifications: async (root, { limit = null, skip = 0, sort = -1 }, { req }, info) => {
      // VALIDATION
      ...
      // QUERY
      const res = await Notification.find({}, null, { sort: { createdAt: sort }, limit, skip })
      return res
    },
    getGlobalNotifications: async (root, { id, limit = null, skip = 0, sort = -1 }, { req }, info) => {
      // VALIDATION
      if (sort !== 1 && sort !== -1) throw new UserInputError(`invalid sort must be 1 or -1`) // CHANGED
      if (!ObjectId.isValid(req.session.userId)) throw new UserInputError(`invalid ID`) // CHANGED - REMOVED CURELY BRACES
      // QUERY
      const res = await Notification.find({ to: null }, null, { sort: { createdAt: sort }, limit, skip })
      return res
    }
  },
  Notification: {
